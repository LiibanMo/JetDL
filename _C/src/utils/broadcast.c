...
alloc_failure:
  if (broadcasted_stridesA)
    UTILS_FREE(broadcasted_stridesA);
  if (broadcasted_stridesB)
    UTILS_FREE(broadcasted_stridesB);
  if (broadcasted_strides_ptrs)
    UTILS_FREE(broadcasted_strides_ptrs);
  return NULL;
}
...
size_t *utils_broadcast_get_result_shape(const size_t *shapeA,
  ...
  if (optype == MATMUL) {
    if (ndimA == 1) {
      ...
      fprintf(stderr,
              "utils_broadcast_get_result_shape: both ndims cannot be 1.");
      return NULL;
    }
  }
