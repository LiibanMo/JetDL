...
size_t *utils_reduction_get_shape(const size_t *shape, const size_t ndim,
...
alloc_failure:
  if (result_shape)
    UTILS_FREE(result_shape);
  return NULL;
}

size_t *utils_reduction_get_dest_strides(const size_t *original_shape,
                                         const size_t original_ndim,
...
alloc_failure:
  if (reduction_strides)
    UTILS_FREE(reduction_strides);
  return NULL;
}
