...
void utils_check_axes(const size_t *shape, const size_t ndim, const int *axes,
                      const size_t naxes) {
  ...
  for (size_t i = 0; i < naxes; i++) {
    ...
    const size_t freq =
        utils_get_count(updated_axes, &updated_axis, naxes, sizeof(size_t));
    if (freq > 1) {
...
void utils_check_ops_shapes(const size_t *shapeA, const size_t ndimA,
  ...
  if (ndimA != ndimB || memcmp(shapeA, shapeB, ndimA)) {
    ...
    for (int i = max_ndim - 1; i >= 0; i--) {
      ...
      if (dimA != dimB && dimA != 1 && dimB != 1) {
        py::gil_scoped_acquire acquire;
        throw py::value_error(py::str("operands could not be broadcasted "
                                      "together; incompatible shapes."));
      }
    }
  }
}

void utils_check_dot_shapes(const size_t *shapeA, const size_t ndimA,
                            const size_t *shapeB, const size_t ndimB) {
  if (!(ndimA == 1 && ndimB == 1)) {
...
void utils_check_vecmat_shapes(const size_t *shapeA, const size_t ndimA,
                               const size_t *shapeB, const size_t ndimB) {
  // (N) @ (..., N, P)
  if (!(ndimA == 1 && ndimB > 1)) {
...
void utils_check_matvec_shapes(const size_t *shapeA, const size_t ndimA,
                               const size_t *shapeB, const size_t ndimB) {
  // (..., M, N) @ (N)
  if (!(ndimA > 1 && ndimB == 1)) {
...
void utils_check_matmul_shapes(const size_t *shapeA, const size_t ndimA,
                               const size_t *shapeB, const size_t ndimB) {
  // (..., M, N) @ (..., N, P)
  if (!(ndimA >= 2 && ndimB >= 2)) {
  ...
  for (int i = max_ndim - 3; i >= 0; i--) {
    ...
    if (dimA != dimB && dimA != 1 && dimB != 1) {
      ...
      throw py::value_error(py::str(
          "operands could not be broadcasted together along batch dimensions"));
    }
  }
}
