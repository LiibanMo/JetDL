    ...
    for (size_t i = 1; i < data.size(); i++) {
        ...
        if (current_item_size != first_item_size) {
            throw py::value_error(
                py::str(
                    "input tensor must be consistent; mismatch in sub-list sizes. (expected {}, got {})"
                )
...
size_t python_utils_get_ndim(const py::object& data) {
    ...
    while (python_utils_is_pylist(item)) {
        ...
        if (python_utils_is_pylist(current_list[0])) {
            ...
            throw std::runtime_error(python_utils_dtype_error_message(current_list[0]));
        }
    }
    ...
    for (const auto& item : data) {
        if (python_utils_is_pylist(item)) {
            ...
            *ptr = py::cast<float>(item);
            ...
            throw py::type_error(
                py::str(
                    "init: incorrect data type present in input data; contains type {}."
                )
